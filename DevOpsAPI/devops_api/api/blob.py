...
class AzureBlobHook:
    ...
    def upload_files(self, container_name, blob_dir, files):
        container_client = (
            self.get_client()
        ...
        for file in files:
            with open(file, 'rb') as f:
                ...
                container_client.upload_blob(name=f"{blob_dir}/{tail}",
                                             data=f, blob_type=BlobType.BlockBlob,
                                             overwrite=True)

                logger.info(f"Uploaded to blob store: {file}, {blob_dir}/{tail}")