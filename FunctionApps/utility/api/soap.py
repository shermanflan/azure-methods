...
class OracleFusionHook:
    def __init__(self, user, password, soap_uri, erp_uri):
        if not user or not password:
            raise Exception("SOAP credentials not provided")
        self.user = user
        self.password = password
        self.soap_uri = soap_uri
        self.erp_uri = erp_uri
    ...
    def get_soap_client(self):
        ...
        logging.info("Creating ERP client")

        self.soap_client = Client(wsdl=self.soap_uri,
                                  settings=settings,
                                  transport=transport
                                  )
        return self.soap_client
    ...
    def get_erp_client(self):
        ...
        logging.info("Creating SOAP client")

        self.erp_client = Client(wsdl=self.erp_uri,
                                 settings=settings,
                                 transport=transport)
    ...
    def download_ess_job_execution_details(self, request_id,
        ...
        documents = []
        for a in pack:
            logging.debug(f"Attachment: {a.DocumentId}, {a.DocumentName}, {a.DocumentTitle}")
    ...
    def get_search_results(self, query):
        ...
        logging.debug(f"service: {search_service}")
    ...
    def get_file(self, document_id, output_folder):
        ...
        logging.debug(f"service: {file_service}")
